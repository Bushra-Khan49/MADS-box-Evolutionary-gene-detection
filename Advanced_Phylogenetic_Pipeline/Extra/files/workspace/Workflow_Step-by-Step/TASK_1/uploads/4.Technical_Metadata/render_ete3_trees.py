...
ANN_FILE = "/Users/bushrakhan/Desktop/NIPGR-data/NIPGR_WORK/MIKCc_Subclade_Annotation_Table.tsv"
WORK_DIR = "/Users/bushrakhan/Desktop/NIPGR-data/MADS-box-Evolutionary-gene-detection/Workflow_Step-by-Step/TASK_1/uploads"
OUT_DIR  = os.path.join(WORK_DIR, "Colored_Tree_Images")
os.makedirs(OUT_DIR, exist_ok=True)

TREES_TO_GENERATE = [
    ("Rice_ClustalO_ML.treefile", "Rice_Full_Proteome"),
    ("Rice_MADS_Domain_ML.treefile", "Rice_MADS_Domain"),
    ("Rice_Kbox_Domain_ML.treefile", "Rice_Kbox_Domain")
]
...
def make_pastel(hex_str, factor=0.6):
    """Fades a hex color by blending it with white"""
...
def draw_tree(title_name, nwk_path, species_ann):
    try:
    ...
    title = TextFace(f" {title_name.replace('_',' ')} — Circular MIKCc Tree", fsize=20, bold=True)
    ...
    for leaf in t.get_leaves():
        ...
        if raw and raw.startswith("AT"):
            ...
            if base in AT_TO_CLADE:
                ...
                base = base.split(".")[0]
                base_clean = base.split("_")[0] # for concatenated string like AT1GXXXXX_1_20
                clade = AT_TO_CLADE.get(base_clean, "Unknown")
        else:
    ...
    out = os.path.join(OUT_DIR, f"{title_name}_circular_colored.png")
    ...
    try:
        ...
        box_width = 800
        box_height = 100 + (len([c for c in CLADE_COLORS if c!="Unknown"])) * 60
        margin = 100
...
for tree_file, title in TREES_TO_GENERATE:
    print(f"\n── {title}")
    nwk = os.path.join(WORK_DIR, tree_file)
    if not os.path.exists(nwk):
        print(f"  Newick not found: {nwk}")
        continue
    
    draw_tree(title, nwk, all_ann)
