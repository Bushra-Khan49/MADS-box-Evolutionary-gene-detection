...
for tree_path, species in tasks:
    if not os.path.exists(tree_path):
        print(f"FAILED: {tree_path} not found")
        continue
    
    try:
        t = Tree(tree_path, format=1)
    except:
        print(f"FAILED: Could not parse {tree_path}")
        continue
        ...
        if len(targets) >= 2:
            ...
            if len(children) == 2:
                ...
                if len(c1) == 1 and len(c2) >= 2:
                    print(f"- **{clade_name}**: Basal:  | Expanded: {c2}")
                elif len(c2) == 1 and len(c1) >= 2:
                    print(f"- **{clade_name}**: Basal:  | Expanded: {c1}")
                else:
                    print(f"- **{clade_name}**: Expanded: {targets}")
            else:
